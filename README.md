...
2. Infrastructure-репозиторий закрепляет образ по `sha256` digest и разворачивает
   его в `develop`.
3. После проверки тег `vX.Y.Z` ставится на уже опубликованный из `develop`
   коммит, доступный из `main`. CI добавляет semver-тег к проверенному digest, не
   пересобирая образ; если соответствующего develop-образа нет, release
   завершается ошибкой.
